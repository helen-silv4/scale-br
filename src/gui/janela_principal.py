...
class JanelaPrincipal:
    ...
    def _construir_area_importacao(self, pai):
        ...
        self.txt_resumo = ctk.CTkTextbox(
            frame, height=90, font=ctk.CTkFont(size=15, family="Courier")
        )
    ...
    def _preencher_valores_padrao(self):
        ...
        for processo, entrada in self.campos_transformadores.items():
            ...
            valor = next(
                (v for k, v in valores_padrao.items() if k in chave or chave in k),
                None,
            )
            if valor:
    ...
    def _construir_area_resultados(self, pai):
        ...
        self.txt_resultados = ctk.CTkTextbox(
            self.frame_resultados, height=220, font=ctk.CTkFont(size=15, family="Courier")
        )
    ...
    def _atualizar_grafico(self):
        ...
        ax.bar(processos, valores, color=cor_barra, edgecolor=cor_borda, linewidth=0.8)
        ax.set_ylabel("Emergia (sej)", color=cor_texto, fontsize=13)
        ax.set_title("Fluxos de Emergia por Processo", color=cor_texto, fontsize=15, pad=10)
        ax.tick_params(colors=cor_texto, labelsize=12)
        ax.yaxis.get_offset_text().set_color(cor_texto)
            ...
            if caminho:
                ...
                if projeto["resultados"]:
                    self.solver = SolverEmergia(
                        self.matriz.obter_matriz(), projeto["transformadores"]
                    )
